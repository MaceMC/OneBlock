...
public class BlockListener extends OneBlockListenerGroup {
	...
	public void onBlockBreak(BlockBreakEvent e) {
		...
		if (ownerString.equalsIgnoreCase("server"))
			handleServerOneBlock(e);
		else
			handlePlayerOneBlock(e, block, ownerString);
	}
	...
	private void handleAction(String action, Block block) {
		...
		if (material != null && material.isBlock()) {
			Common.runLater(1, () -> block.setType(material, false));
			return;
		}
		...
		if (entity != null && entity.isSpawnable()) {
			Common.runLater(1, () -> {
				...
				block.getWorld().spawnEntity(block.getLocation().toCenterLocation().add(0, 0.5, 0), entity, CreatureSpawnEvent.SpawnReason.CUSTOM);
			});
			return;
		}

		Common.runLater(1, () -> block.setType(Material.GRASS_BLOCK, false));
	}
